...
// Match <path d="..." title="..." id="..." />
// The path tags might be formatted in various orders or multiline, so we search globally
const countryRegex = /<path[^>]+d="([^"]+)"[^>]+title="([^"]+)"[^>]+id="([^"]+)"/g;
let match;
const countries = [];

// Alternate regex in case order of attributes is different
const alternateRegex = /<path[^>]+d="([^"]+)"[^>]+id="([^"]+)"[^>]+title="([^"]+)"/g;

// Also look for other attributes if order changes
// We'll write a simple tokenizer/extractor for paths to be robust
